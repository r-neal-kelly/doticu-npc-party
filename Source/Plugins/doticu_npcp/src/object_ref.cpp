...
namespace doticu_npcp { namespace Object_Ref {
    ...
    void Move_To_Orbit_And_Callback(Reference_t* ref, Reference_t* origin, Float_t radius, Float_t degree, Virtual_Callback_i** callback)
    {
        NPCP_ASSERT(ref);
        NPCP_ASSERT(origin);
        ...
        public:
            ...
            Float_t radius;
            Float_t degree;
            Arguments(Reference_t* ref, Reference_t* origin, Float_t radius, Float_t degree) :
                ref(ref), origin(origin), radius(radius), degree(degree)
            {
            }
            Bool_t operator()(Arguments_t* arguments)
            {
                ...
                arguments->At(1)->Pack(origin);
                arguments->At(2)->Float(radius);
                arguments->At(3)->Float(degree);
                return true;
            }
        } arguments(ref, origin, radius, degree);

        Virtual_Machine_t::Self()->Call_Method(Consts::Funcs_Quest(), "doticu_npcp_funcs", "Move_To_Orbit", &arguments, callback);
    }
                 ...
                 Virtual_Callback_i** callback)
    {
        ...
        public:
            ...
            Float_t z_offset;
            Bool_t do_match_rotation;
            Arguments(Reference_t* target,
                      Float_t x_offset,
                      Float_t y_offset,
                      Float_t z_offset,
                      Bool_t do_match_rotation) :
                target(target), x_offset(x_offset), y_offset(y_offset), z_offset(z_offset), do_match_rotation(do_match_rotation)
            {
            }
            Bool_t operator()(Arguments_t* arguments)
            {
                arguments->Resize(5);
                arguments->At(0)->Pack(target);
                arguments->At(1)->Float(x_offset);
                arguments->At(2)->Float(y_offset);
                arguments->At(3)->Float(z_offset);
                arguments->At(4)->Bool(do_match_rotation);
                return true;
            }
        } arguments(target, x_offset, y_offset, z_offset, do_match_rotation);

        Virtual_Machine_t::Self()->Call_Method(ref, "ObjectReference", "MoveTo", &arguments, callback);
    }
                   ...
                   Virtual_Callback_i** callback)
    {
        ...
        public:
            ...
            {
                arguments->Resize(3);
                arguments->At(0)->Float(x_degree);
                arguments->At(1)->Float(y_degree);
                arguments->At(2)->Float(z_degree);
                return true;
            }
        } arguments(x_degree, y_degree, z_degree);

        Virtual_Machine_t::Self()->Call_Method(ref, "ObjectReference", "SetAngle", &arguments, callback);
    }
